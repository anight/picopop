...
/*
 * Check one surface against its reference pixels.
 *
 * `what` names the record for messages. It is a surface rather than a
 * (set, index) pair because the optional graphics are not reachable through
 * any set's image list - see the note on OPTGRAF in convert.py - and are
 * looked up by resource id instead.
 */
/*
 * The environment sprite set's shpl id. The optional graphics take its palette
 * row; convert.py's OPTGRAF_PALETTE_SET is the same constant.
 */
#define OPTGRAF_PALETTE_SET 200

/* Look up a converted image by its resource id, the way the game does for the
 * optional graphics (find_resource_surface() in seg009.c). */
static const SDL_Surface *find_surface(const char *datfile, int resource_id)
{
	for (unsigned i = 0; i < datfiles_count; ++i) {
		if (strcmp(datfiles[i].filename, datfile) != 0)
			continue;
		for (unsigned j = 0; j < datfiles[i].resources_count; ++j)
			if (datfiles[i].resources[j]->resource_id == resource_id)
				return datfiles[i].resources[j]->surface;
	}
	return NULL;
}

static int check_surface(const SDL_Surface *src, const char *what,
                         unsigned row_base, int w, int h,
                         const unsigned char *want)
{
	...
	if (src->w != w || src->h != h) {
		printf("  SIZE    %s: have %dx%d, want %dx%d\n",
		       what, src->w, src->h, w, h);
		return 0;
	}

	/* The colour key must be this set's row base, or transparency picks the
	 * wrong index once the sprite is drawn over something. */
	if (src->colorkey != (Uint32)row_base) {
		printf("  KEY     %s: key %u, row base %u\n",
		       what, (unsigned)src->colorkey, row_base);
		return 0;
	}
...
int main(void)
{
	...
	for (Uint32 n = 0; n < count; ++n) {
		...
		if (index == 0xFFFFFFFFu) {
			/*
			 * An optional-graphics image: shpl_id is its own resource id.
			 * It takes the environment set's palette row, which is what
			 * load_more_opt_graf() colours it with in the game.
			 */
			const struct sprite_set_s *env = find_set(datfile, OPTGRAF_PALETTE_SET);
			if (env == NULL) {
				printf("  NO SET  %s res%u (optional graphics)\n",
				       datfile, (unsigned)shpl_id);
				missing++;
				continue;
			}
			if (env != cur) {
				load_set_palette(env);
				cur = env;
			}
			snprintf(what, sizeof(what), "%s res%u (optgraf)",
			         datfile, (unsigned)shpl_id);
			checked++;
			optgraf_checked++;
			if (!check_surface(find_surface(datfile, (int)shpl_id), what,
			                   env->palette_row * 16, (int)w, (int)h, buf))
				mismatched++;
			continue;
		}
		...
		snprintf(what, sizeof(what), "%s res%d image %u",
		         set->datfile, set->shpl_resource_id, index);
		checked++;
		if (!check_surface(index < set->n_images ? set->images[index] : NULL,
		                   what, set->palette_row * 16, (int)w, (int)h, buf))
			mismatched++;
	}
	...
	printf("\n%d sprites checked (%d optional graphics), %d mismatched, "
	       "%d with no sprite set\n",
	       checked, optgraf_checked, mismatched, missing);
